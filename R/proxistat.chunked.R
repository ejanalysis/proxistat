...
#'   Each saved output is a vector of proximity scores if FUN=get.proxistat, or matrix with extra columns depending on return. parameters above.
#' @export
proxistat.chunked <- function(frompoints, topoints, fromchunksize, tochunksize, startchunk=1, FUN=proxistat, area, folder=getwd(), savechunks=TRUE, assemble=TRUE, ...) {
