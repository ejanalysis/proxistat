...
#' @param return.units A string that is 'miles' by default, or 'km' for kilometers, 
...
#' @export
get.distances.all <- function(frompoints, topoints, return.units='miles', return.crosstab=FALSE, return.rownums=TRUE, return.latlons=TRUE) {
  #require(sp)
  km.per.mile <- 1.60934
  ...
  if (return.units=='miles') {results.matrix <- results.matrix / km.per.mile }
