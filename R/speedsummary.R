...
speedsummary <- function(start, end, n) {
  ...
  perhour <- round((dim(rnd.data)[1] / as.numeric(total.seconds))*3600,0)
  print(paste0(
    format(round((dim(rnd.data)[1] / as.numeric(total.seconds))*3600,0), big.mark=',', scientific=FALSE), 
    ' buffers per hour (',
    format(dim(rnd.data)[1],big.mark = ',', scientific = FALSE),
    ' lat/long pairs per ',
    format(round(as.numeric(total.seconds),0), big.mark = ',', scientific = FALSE),
    ' seconds)'
  ))
  print(round(total.benchmark,1))
  cat('Note that results were unavailable for', sum(sapply(dta.list, FUN = is.null)),'out of the', length(dta.list), 'buffers.', '\n')
  return(perhour)
  }