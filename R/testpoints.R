...
#' 
#' @return By default, returns a dataframe that has 2 columns: lat and lon, in decimal degrees, with 1 row per point.
...
#'     
#' @export
testpoints <- function(n, minlat=40,maxlat=42, minlon=-125,maxlon=-70) { 
  is.wholenumber <- function(x, tol = .Machine$double.eps^0.5)  abs(x - round(x)) < tol
  if (missing(n) || !is.numeric(n) || is.na(n) || !is.wholenumber(n) || is.infinite(n) || n < 0 || n > 5e7) {
    stop('n must be specified, and must be a whole number between 0 and 50 million')
  }
